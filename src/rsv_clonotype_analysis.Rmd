...
abstract: Here you can find how the processing and plotting was performed for the paper entitled "Multivalent Antigen Display on Nanoparticles Elicits Diverse and Broad B cell Responses"
knit: (function(i20-merutFile, encoding) {
          rmarkdown::render(i20-merutFile,
                            encoding = encoding, 
                            output_file = paste0(
                              xfun::sans_ext(i20-merutFile), '_', Sys.Date(), '.html'),
...
```{r load_rds}
#clonotype_full_fixed <- readRDS("../data/clonotypes/individualized/processed_clonotypes.rds")
...
rds_summary <- rds_merge %>%
  ...
  mutate(clonal_size_rank = dense_rank(desc(clonal_size))) %>%
  ungroup() %>%
  distinct()
...
mabs_lors <- read.csv("../../rsv_ki/sanger_QC/sanger_QC_HC/data/sc_all_individualized_v5/v5_sc_summary_filtered.csv")
mabs_lors <- mabs_lors %>%
...
## Diversity single-cell on prism, same as current dbs

## Plot V alleles unique and shared per animal, stacked plot

```{r alelle_comparison}
ls <- list.files("../data/databases/individualized", recursive = T, full.names = T)
ls <- ls[grepl("V.fasta", ls)]
names(ls) <- basename(dirname(ls))
...
for(i in seq_along(mabs_of_interests)){
  ...
  fasta <- unique(scifer::df_to_fasta(sequence_strings = filtered$VDJ_nt, sequence_name = gsub(":", "_",filtered$new_name)))
  ...
  if(any(stringr::str_count(names(mab), "LOR") > 1)){
    ...
    sc_seqs <- scifer::df_to_fasta(sequence_strings = sc_seqs$VDJ_nt, sequence_name = gsub(":", "_",sc_seqs$new_name))
